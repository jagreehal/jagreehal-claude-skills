...
# Automatic Code Review Plugin
# Logs file modifications and triggers review on session stop
...
log_event() {
  ...
  local log_file="/tmp/code-review-log-${session_id}.jsonl"
  touch "$log_file"
...
get_or_initialize_plugin_settings() {
  ...
  if [[ ! -f "$settings_file" ]] || ! jq -e '.codeReview' "$settings_file" >/dev/null 2>&1; then
    ...
    if [[ ! -f "$init_flag" ]]; then
      ...
      echo "$existing" | jq '.codeReview = {"enabled": true, "fileExtensions": ["ts", "tsx"], "rulesFile": ".claude/code-review/rules.md"}' > "$settings_file"
      ...
      echo "   Customize rules for your project." >&2
...
INSTRUCTION: Use the Task tool with subagent_type "code-reviewer".
Pass only the file list as the prompt.
The agent will follow its configured review procedure.

After receiving review results:

1. Show ALL findings to the user

2. Evaluate each finding. Valid reasons to skip:
   - IMPOSSIBLE: Tried to fix, cannot satisfy feedback + requirements + lint + tests
   ...
   NEVER valid excuses:
   - "Too much time" - do the work
   - "Out of scope" - if you touched it, it's in scope
   - "Inconsistent with existing" - fix existing code too
   - "Pre-existing code" - present value/effort, let user decide
   - "Would require large refactor" - present value/effort, let user decide

3. For each finding:
   - NO VALID SKIP REASON -> fix it
   - VALID SKIP REASON -> skip with specific justification

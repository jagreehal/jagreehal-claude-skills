...
1. User runs `/workflow <mode>` to activate a mode
2. You announce the mode at the start of EVERY message while active
3. You follow that mode's behavior rules exactly
4. User can switch modes anytime with `/workflow <other-mode>`
5. User runs `/workflow off` to deactivate and return to normal

## Announcement Format

Start every message with:
```
WORKFLOW_MODE: <emoji>:<NAME>
```
...
- **tdd-workflow:** In PAIR mode, follow TDD cycles strictly
- **fn-args-deps:** In DEEP-THINK mode, research and recommend dependency patterns
- **create-tasks:** In STEP-BY-STEP mode, break work into small, discussable tasks

## Rules

1. Always announce mode at start of every message
2. Follow mode's behavior rules exactly
3. Don't mix modes - one mode active at a time
4. User controls mode switching
5. When mode is off, return to normal behavior
